...
public static class RankedTargetTableRenderer
{
    public static void Write(IReadOnlyList<RankedTarget> results, TextWriter writer)
    {
        ...
        writer.WriteLine("Rank | Show                          | Lang | Score | Reach | Frequency | NicheFit | Activity | NewestEp   | Contact");
        writer.WriteLine("-----+-------------------------------+------+-------+-------+-----------+----------+----------+------------+----------------------------");
        for (var i = 0; i < results.Count; i++)
        {
            ...
            writer.WriteLine(
                $"{(i + 1),4} | {TrimTo(r.ShowName, 29),-29} | {r.DetectedLanguage,-4} | {FormatDecimal(r.Score),5} | {FormatDecimal(r.Reach),5} | {FormatDecimal(r.Frequency),9} | {FormatDecimal(r.NicheFit),8} | {FormatDecimal(r.ActivityScore),8} | {newestEpisode,-10} | {TrimTo(r.ContactEmail ?? "-", 26)}");
        }
    }
