...
public sealed class CliArgumentParserTests
{
    ...
    {
        var result = CliArgumentParser.TryParse(["--search-terms", "ai", "--keywords", "ai,startups", "--published-after-days", "30", "--use-mock"]);

        Assert.True(result.Success);
        Assert.NotNull(result.Payload);
        Assert.Single(result.Payload!.SearchTerms);
    ...
    {
        var result = CliArgumentParser.TryParse(["--search-terms", "ai", "--keywords", "ai,startups", "--published-after-days", "30", "--verbose"]);
    ...
    public void TryParse_ParsesReachBounds_WhenProvided()
    {
        var result = CliArgumentParser.TryParse(
            ["--search-terms", "ai", "--keywords", "ai,startups", "--published-after-days", "30", "--min-reach", "0.25", "--max-reach", "0.9"]);
    ...
    {
        var result = CliArgumentParser.TryParse(
            ["--search-terms", "ai", "--keywords", "ai,startups", "--published-after-days", "30", "--min-reach", "1.2"]);
    ...
    {
        var result = CliArgumentParser.TryParse(
            ["--search-terms", "ai", "--keywords", "ai,startups", "--published-after-days", "30", "--min-reach", "0.8", "--max-reach", "0.3"]);
    ...
    [Fact]
    public void TryParse_Fails_WhenSearchTermsAreMissing()
    {
        var result = CliArgumentParser.TryParse(["--keywords", "ai,startups", "--published-after-days", "30"]);

        Assert.False(result.Success);
        Assert.Contains(result.Errors, e => e.Code == "missing_search_terms");
    }
}
